...
class CuDNNLSTM_MDN:
  def __init__(self, n_verts=15, n_dims=3, n_mixes=2, look_back=1, cells=[32,32,32,32], use_mdn=True, lr=1e-3):
    self.n_verts = n_verts
    self.n_dims = n_dims
    self.n_mixes = n_mixes
    self.look_back = look_back
  ...
  def build_model(self):
    i = Input((self.look_back, self.n_verts*self.n_dims))
    h = self.CuDNNLSTM(self.cells[0], return_sequences=True)(i) # return sequences, stateful
    h = self.CuDNNLSTM(self.cells[1], return_sequences=True)(h)
    h = self.CuDNNLSTM(self.cells[2])(h)
    h = Dense(self.cells[3])(h)
    if self.use_mdn:
      o = MDN(self.n_mixes, self.n_verts*self.n_dims)(h)
    else:
